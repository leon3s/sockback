...
class Sockback {
  ...
  _loadModelsDirectories(namespaceDirectory) {
    this.socknet.app.models().forEach(({ modelName }) => {
      const modelDirectory = path.join(namespaceDirectory, modelName);
  ...
  enable(serverCreator, callback) {
    this._app = serverCreator(this.server);
